...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!authResponse.ok) {
      ...
      console.error('Key ID format check:', keyId.length, 'chars');
    ...
    // Add "Legal Opinion/" folder prefix to organize files
    const fileName = `Legal Opinion/${Date.now()}-${file.name}`;
    ...
    // Use bucket name instead of bucket ID for download URL
