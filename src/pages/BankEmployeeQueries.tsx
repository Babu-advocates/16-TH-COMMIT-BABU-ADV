    ...
    try {
      ...
      if (files.length > 0) {
        ...
        for (const file of files) {
          ...
          const res = await fetch(`${SUPABASE_URL}/functions/v1/upload-to-backblaze`, {
            method: 'POST',
            headers: {
          ...
          if (!res.ok) {
            ...
            throw new Error(`Backblaze upload failed: ${res.status} ${res.statusText} - ${errText}`);
          }

          const json: any = await res.json();
          if (!json?.success || !json?.file?.download_url) {
